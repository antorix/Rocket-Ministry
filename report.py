...
class Report(object):
    ...
    def __init__(self):
        self.hours = app.RM.settings[2][0]
        self.credit = app.RM.settings[2][1]
        self.placements = app.RM.settings[2][2]
        self.videos = app.RM.settings[2][3]
        self.returns = app.RM.settings[2][4]
        self.studies = app.RM.settings[2][5]
        self.startTime = app.RM.settings[2][6]
        self.endTime = app.RM.settings[2][7]
        self.reportTime = app.RM.settings[2][8]
        self.difTime = app.RM.settings[2][9]
        self.note = app.RM.settings[2][10]
        self.reminder = app.RM.settings[2][11]
        self.lastMonth = app.RM.settings[2][12]
    ...
    def saveReport(self, message="", mute=False, save=True, forceNotify=False):
        ...
        if mute == False:
            app.RM.log(message, forceNotify=forceNotify)
            ...
            app.RM.resources[2].insert(0, f"\n{date} {time2}: {message}")
        if save==True:
            app.RM.save(backup=True, silent=True)

    def checkNewMonth(self, forceDebug=False):
        savedMonth = app.RM.settings[3]
        ...
        if savedMonth != currentMonth or forceDebug == True:
            ...
            app.RM.settings[3] = time.strftime("%b", time.localtime())
            self.reminder = 1
    ...
    def toggleTimer(self):
        ...
        if self.startTime == 0:
            ...
            result = 1 if app.RM.settings[0][2] == 0 else 2 # если в настройках включен кредит, спрашиваем
        return result
    ...
    def getCurrentHours(self):
        ...
        app.RM.settings[0][3] / utils.days()
        if app.RM.settings[0][3] == 0:
            gap_str = ""
        elif gap >= 0:
    ...
    def saveLastMonth(self):
        ...
        # Calculate rollovers
        if app.RM.settings[0][15]==1: # rollover seconds to next month if activated
        ...
        if int(time.strftime("%m", time.localtime())) == 10: 
            app.RM.settings[4] = [None, None, None, None, None, None, None, None, None, None, None, None]
        
        # Save last month hour+credit into service year
        app.RM.settings[4][utils.monthName()[7]-1] = self.hours + self.credit
    ...
    def modify(self, input=" "):
        ...
        if input == "(":  # start timer
            ...
            forceNotify = True if app.RM.settings[0][0] == 1 else False
    ...
    def getCurrentMonthReport(self):
        ...
        result = utils.filterOutFormatting(result)
        return result

    def getLastMonthReport(self):
        """ Выдает отчет прошлого месяца """
        self.lastMonthNoFormatting = utils.filterOutFormatting(self.lastMonth)
