...
class CircularTimePicker(BoxLayout):
    ...
    # deleted "AM" and "PM":
    ampm_format = StringProperty("[color={am_color}][ref=am][/ref][/color]\n[color={pm_color}][ref=pm][/ref][/color]")
    ...
    def _get_time_text(self):
        self.selector_color = [app.RM.titleColor[0], app.RM.titleColor[1], app.RM.titleColor[2]] if app.RM.theme != "purple" \
            else [.56, .42, .77]
    ...
    def _switch_picker(self, *a, **kw):
        ...
        if noanim:
            ...
            if prevpicker in container.children:
                anim = Animation(scale=1.5, d=.2, t="in_back") & Animation(opacity=0, d=.2, t="in_cubic")
                anim.start(prevpicker)
                Clock.schedule_once(lambda *a: container.remove_widget(prevpicker), .3)#.31)
            picker.scale = 1
            picker.opacity = 0
            if picker.parent:
                picker.parent.remove_widget(picker)
