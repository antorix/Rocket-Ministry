...
Запуск программы подразумевает наличие в системе интерпретатора Python. Если что-то идет не так и он не устанавливается, попробуйте установить официальную версию с сайта [python.org](https://python.org) либо из магазина Windows Store (сделайте поиск «Python 3.10»). Последний способ самый простой.

Если Python успешно установлен в системе, для запуска Rocket Ministry просто кликните по файлу *Rocket Ministry.pyw*. Запустится автоматическая загрузка остальных файлов приложения – нужно подождать буквально несколько секунд. Для этого необходима связь с Интернетом.
...
Если что-то идет не так, не работает, непонятно и т. д., [пишите мне](https://github.com/antorix/Rocket-Ministry/blob/master/README.md#обратная-связь), я оперативно отвечаю.
