...
Этот раздел не претендует на исчерпывающую документацию (кто их читает?), но я постался объяснить некоторые моменты, которые не столь очевидны из интерфейса. Если вы хотите использовать потенциал программы по максимуму, крайне рекомендую прочитать этот раздел.
...
**Технические статусы**. На самом деле технически существует еще два вида статусов, но их нельзя присвоить вручную:
...
Эта функция по умолчанию отключена, но если ее включить, можно будет быстро задавать и менять статус квартиры прямо при записи посещения. В конце записи должна стоять цифра от 0 до 6, обозначающая статус. 0 = отказ, 1 = интерес и т. д. Например:

`поговорили о Царстве Бога, прийти через неделю. 1`

Такая нотация автоматически присвоит данной квартире или контакту статус «интерес». Поставьте 0, и будет отказ. Это более быстрый способ назначения статусов, чем вручную из деталей квартиры. Это должен быть именно *последний* символ строки. Если символа нет, квартира получит неопределенный статус (❔).
...
**Отдельные контакты**. Любой привязанный контакт можно скопировать в отдельный – создастся его полная копия со всеми параметрами и посещениями, только уже отдельная. Такой контакт существует как бы сам по себе, у него можно свободно изменять адрес (в его деталях есть отдельный пункт для этого). Также учтите, что при удалении участка вместе с ним удаляются и все живущие на нем контакты-квартиры, что логично. Однако программа при удалении участка автоматически создает их отдельные копии, так что с точки зрения пользователя все интересующиеся как бы продолжают «жить» и после сдачи участка, пока не будут удалены вручную.

Еще одно важное отличие между отдельными и привязанными контактами: первые показаны вне зависимости от статуса. Если жильцы на участках с отказом в разделе *Контакты* не отображаются, отдельные контакты будут там даже с отказами.
