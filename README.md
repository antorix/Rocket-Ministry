...
Затем убедитесь, что в системе установлен Python. Это можно увидеть по наличию его иконок в списке программ (*Python 3.10*, *IDLE* и др.). Если Python установлен, то простой клик по файлу *Rocket Ministry.py* обязательно приведет к старту приложения. Это произойдет или сразу, или спустя несколько секунд, если нужно докачать файлы программы. Файл *Rocket Ministry.py* определяет, есть ли файлы программы в наличии, и если нет, скачивает установщик для их докачки (нужна связь с Интернетом), а затем запускает приложение. Если ничего не нужно скачивать, он просто запускает приложение.
...
В контекстном меню файла main.py можно выбрать запуск по умолчанию в Python 3.10 и создать для него иконку на рабочем столе. Тогда можно быстро запускать с рабочего стола одним кликом. Если вам нужна красивая PNG-иконка для ярлыка, ее можно взять [здесь](https://flyclipart.com/rocket-icon-captiva-iconset-bokehlicia-rocket-icon-png-246987).
