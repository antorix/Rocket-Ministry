...
Если что-то пошло не так, попробуйте более традиционный метод установки Python-приложения. Скачайте и установите движок Python из [Магазина Microsoft](https://apps.microsoft.com/store/detail/python-37/9NJ46SX7X90P) (рекомендую Python 3.7). Затем скачайте и распакуйте архив с файлами Rocket Ministry: [master.zip](https://github.com/antorix/Rocket-Ministry/archive/refs/heads/master.zip). Запустите файл *Rocket Ministry.py*. Программа должна запуститься.

### Возможные проблемы при установке на Windows

Несколько советов и технических деталей на случай, если оба вышеописанных способа ни к чему не привели. Убедитесь, что перед запуском установки папка *Rocket Ministry* распакована и помещена в место, где вам будет удобно ее держать. Лучше всего – рабочий стол или документы. Не запускайте установку внутри архива, что может не сработать. Даже если это сработает, программа попадет во временную папку (обычно это `AppData\Local\Temp\_tc\`), где вам будет сложно ее найти.

Затем убедитесь, что в системе установлен Python. Это можно увидеть по наличию его иконок в списке программ (*Python 3.7*, *IDLE* и др.). Если Python установлен, то простой клик по файлу *Rocket Ministry.py* обязательно приведет к старту приложения. Это произойдет или сразу, или спустя несколько секунд, если нужно докачать файлы программы. Файл *Rocket Ministry.py* является веб-установщиком и лончером одновременно: сначала он определяет, есть ли файлы программы в наличии, и если нет, скачивает их (нужна связь с Интернетом), а затем запускает приложение. Если ничего не нужно скачивать, он просто запускает приложение.

Перед первым запуском программы автоматически создаются две иконки: на рабочем столе и в списке программ. В дальнейшем Rocket Ministry можно запускать по любой из этих иконок. Также выполняется установка шрифта *Liberation Mono*, с котором настольная версия Rocket Ministry выглядит наиболее адекватно. При установке шрифта может потребоваться ваше подтверждение.
...
Если по-прежнему не удается решить проблемы, [пишите мне по почте или в Telegram](https://github.com/antorix/Rocket-Ministry/blob/master/README.md#обратная-связь), я оперативно отвечаю.
