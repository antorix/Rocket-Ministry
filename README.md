...
При запуске Rocket Ministry ищет одну из двух графических библиотек: SL4A (для Android) либо Tkinter (для настольных ОС). Если она не находит ни ту, ни другую, она переходит в консольный режим без графики. В этом режиме программа способна работать на любых устройствах, платформах и архитектурах, где есть Python со стандартными библиотеками.

Работа в консольном режиме практически идентична графическому с той лишь разницей, что для выбора пункта меню нужно ввести его номер и нажать на Enter. На экране подъезда для входа в квартиру нужно ввести номер квартиры, а для создания новой квартиры нужно вначале написать знак +. На экране есть ряд подсказок.

Вот более полный список команд консольного режима на экране подъезда:

`+1` – создание одной квартиры

`+1-50` – создание диапазона квартир

`01` – простановка отказа в квартиру, не заходя в нее (пишем 0 и сразу номер квартиры, без пробела)
